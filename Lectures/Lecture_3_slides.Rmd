...
4. Theoretical probability models
...
- How can we arrive here using the axioms?

  1. $P(E_i) = \frac{1}{n} \forall\;i = \{1,2,\dots,n\}$ because
  
  2. $P(S) = P(E_1 \cup E_2 \cup \dots E_n) = P(E_1) + P(E_2) + \dots P(E_n) = 1$ (Axiom 3, then Axiom 2)
  
  3. $P(E_1) + P(E_2) + \dots P(E_n) = nP(E_1) = 1$ (definition of equiprobability)
  
  4. $P(E_1) = \frac{1}{n}$ and $P(E_i) = \frac{1}{n} \forall \; i = \{1,2,\dots,n\}$
  
  5. $P(A) = \sum_{E_i \in A} P(E_i)$ (Axiom 3 again)
  
  6. $P(A) = \sum_{E_i \in A}\frac{1}{n} = \frac{|A|}{n} \blacksquare$
...
- We can specify and manifuplate formulas describing random variables using the **expectations operator**
...
- The expected value concent can be applied to any **function of a random variable**
...
```{r}
data.frame(py = dunif(seq(-5,5,length.out = 100),min = -3,max = 4),y = seq(-5,5,length.out = 100)) %>%
  ggplot(aes(x = y,y = py)) + 
  geom_line() + 
  labs(x = 'y',y = 'f(y)')
```
...
- What is the probability $Y$ takes on some value $y$ within an interval between $a$ and $b$?
